...
$(function() {
	$("form").validate({
		...
		submitHandler : function(form) {// 必须写在验证前面，否则无法ajax提交
			...
			//维修人员不能为空
			if($("#inspectman").val().trim()==""){
				layer.msg("请选择维修人员!");
				return;
			}
			if($("#starts").val().trim()=="点击选择开始时间"){
				layer.msg("请选择开始时间!");
				return;
			}
			if($("#stops").val().trim()=="点击选择结束时间"){
				layer.msg("请选择结束时间!");
				return;
			}
			var b=checkDateTime($("#starts").val(),$("#stops").val());
			if(!b){
				layer.msg("结束时间不能小于开始时间!");
				return;
			}
			//维护过程不能为空
			if($("#inspectinfo").val().trim()==""){
				layer.msg("请录入维护过程!");
				return;
			}
			...
			$("#maintainfee").val(totalconsumablefee+totalpartfee);
