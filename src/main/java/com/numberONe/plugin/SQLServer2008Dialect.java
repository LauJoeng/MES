...
public class SQLServer2008Dialect {
	...
	private String getLimitString(String querySqlString, int offset, int limit,
		...
		sb.insert(0, "WITH query AS (")
		.append(") select top ("+limit+") * from query where id not in (select top("+offset+") id from query)");
		
		/*sb.append("WHERE id BETWEEN ").append(offset + 1)
	...
	protected static void insertRowNumberFunction(StringBuilder sql,
		...
		sql.insert(selectEndIndex,orderby);
	}
