...
(function() {
	lyGrid = (function(params) {
		var confs = {
			...
			width : '100%', // 表格高度
			height : '100%', // 表格宽度
			theadHeight : '40px', // 表格的thead高度
			tbodyHeight : '60px',// 表格body的每一行高度
			jsonUrl : '', // 访问后台地址
		...
		var cBodytb = function(divId,jsonData){
			...
			for(;d<e;d++){
				...
				if(CommnUtil.notNull(rowdata)){
					...
					td_d.setAttribute("style","vertical-align: middle;");
					$.each(column, function(o) {
						if (!column[o].hide || column[o].hide == undefined) {
							var td_o = tr.insertCell(-1);
							td_o.className=column[o].tbodyClass;
							td_o.setAttribute("style", "text-align:" + column[o].align + ";width: " + column[o].width + ";vertical-align: middle;word-wrap:break-word;word-break: keep-all;white-space: normal;text-overflow: ellipsis;");
